...
for (const spec of [
  ...
  if (spec.name !== "laptop") {
    ...
    check(
      ...
      b.position[0] > a.position[0] + 30 && b.position[2] < a.position[2] - 30,
    );
    ...
    await page.touchscreen.tap(spec.width * 0.5, spec.height * 0.72);
    await page.waitForTimeout(200);
    check(
      spec.name + " tap selects planet",
      (await inspect(page)).selected === "ORPHEUS IV",
    );
    await page
...
await page.getByRole("button", { name: "Plant a star" }).click();
await page.waitForTimeout(300);
check(
  "Visible creation control plants another star",
  (await inspect(page)).seeds === 2,
);
await page.getByRole("button", { name: "Hold to gather" }).hover();
await page.mouse.down();
await page.waitForTimeout(250);
check("Visible gather control activates gravity", (await inspect(page)).field);
await page.mouse.up();
await page.waitForTimeout(250);
check("Visible gather control releases gravity", !(await inspect(page)).field);
await page.getByRole("button", { name: "CREATE", exact: false }).click();
...
check("WebGL2 remains navigable", (await inspect(gl)).position[2] < 8950);
...
await fs.writeFile(
  "artifacts/vastness-devices.json",

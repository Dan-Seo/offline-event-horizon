...
- Gate D accepted the current experience after inspecting local healing/lab captures and the deployed quiet-water sequence, coastal traversal, portrait controls, sensor panels and trajectory plots on both backends. Actual Chrome interactions passed manual takeover, quiet-mode control, rest, continuous coastal contact, touch release and recovery. Quiet compositions remain legible, the craft stays subordinate to the landscape, and the measured journey has no frame interval over 50 ms. This is rendered evidence and automated interaction judgment, not a claim of a human study or a fresh audio evaluation.
- Gate E accepted runtime `24c7e90c7794c6ab0a051fc077cf23cd90727701`: GitHub CI passed; Vercel deployment `dpl_r7QhyNPNoWXv4HUFTZMoohmf3PY9` reached READY and owns the public alias; production WebGPU and WebGL2 lab suites each passed 39 checks; the 120-second quiet-journey suite passed 16 checks; both actual 32-frame exports replayed all 31 relative estimates exactly. No runtime/shader errors were recorded in those successful production runs. Full evidence, measured limits and the initial worker-network limitation are recorded in `docs/QA-3.0-HARDENING.md`.

The release runtime was tested through `https://offline-vastness.vercel.app`; its immutable deployment is `https://offline-event-horizon-i4i6n3qqq-sf-i455.vercel.app`. Subsequent evidence-only documentation commits do not change that deployed runtime. Critical workers were closed after acceptance; only the evidence writer remained until the report was finalized.

## 2026-09-22 Astra / Fable completion pass

The preceding gates are historical. This pass started from clean `main` / `origin/main` at `9e02ad6ecf5c8322084f89313bbf2a3263a64714`, with no competing writer. The existing public alias initially resolved to deployment `dpl_32E7S7VNqbF1fKjuPp7LcE1WHxaK` (September 10), so the September 11 local HEAD was not assumed to be deployed. Astra owns integration branch `release/fable-carry-clarity`, all product writes and this plan. Fable reviews through the user's existing Herdr sibling pane and cannot write product files. No extra workers were launched with limited free RAM; GPU suites run serially.

| Actual problem | User value / difficulty | Minimum scope / verification | Excluded |
| --- | --- | --- | --- |
| Failed search and initial observation were presented as rest; `stops` counted failed searches. LOST retained an old internal pose which the stall detector treated as current. | Stops explain real intention; recovered observations can resume travel. Keep physical rest, sensor cadence and visitor language consistent. | Director, system and one engine assignment; distinct-capture recovery, tracking/segment guard, deterministic failure tests and actual Carry. | New navigation algorithm, ground-truth routes, changed retry/speed/freshness thresholds. |
| Pale glossy hull and bright seams compete with the mirrored water and ringed planet in the existing starting sea. | Keep the craft legible while giving the landscape more visual weight. | Craft material only; same-control manual-rest before/after and live Carry images, same-condition timing. | New geometry, effects, shader passes, destinations or presentation-only scenes. |
| Old short clips leave ATE unavailable, and metric documentation inaccurately describes the global lost-frame gate as per-segment. | Reproducible evidence with an honest accuracy and loss interpretation. | Actual turning RGB-D, an opposite-turn holdout, unscaled SE(3) replay, truth-perturbation worker test and corrected documentation. | New SLAM, scale fitting, per-segment loss reassignment or lower acceptance gates. |

Actual review identity: Claude Code 2.1.278, configured `claude-fable-5-1[1m]`, session `fd835848-842d-4f9b-bfa6-9bc6cdf9304c`; the session reports `claude-fable-5-1` and its existing max setting was preserved. This is execution/session metadata, not independent verification of server internals. Existing OAuth was reused. Astra did not switch authentication, model or effort. TypeSafe was a skill without a connected MCP tool; the user-specified existing credential enabled one bounded official API call to `jev-latest`, which returned `jev-1.13.0` (1,827 input / 95 output tokens). Jev classified the old stop-success claim as unsupported and the proposed split as a correctness repair; this is review triage, not test evidence. Secrets and raw provider logs are not retained in the repository.

Review A / A2 inspected `9e02ad6`. Astra proposed splitting failed holds from scenic rest and allowing fresh safe observations to end a failed search. Fable found the more consequential LOST-pose stall and slow-sensing feedback paths, and required two distinct capture IDs plus a latched stall cause. Astra accepted those findings. Fable withdrew its proposed per-segment loss counter after Astra identified ambiguous segment ownership during reacquisition; the existing global loss accounting remains. A2 accepted this bounded contract. Implementation and its checks are `a75f9f5ad49741d37025b892e126197ff1ae63d9`.

Reviews B (first end-to-end result), C (research/failure evidence) and D (release candidate), deployment identity and remaining limits are recorded with the final results in [the existing QA record](QA-3.0-HARDENING.md). An accepted model opinion never substitutes for a runnable check.

B returned REVISE at `a75f9f5`: beauty watching could overwrite a failed-search hold, and a stale render interval could erase confirmation between fresh observations. Both were reproduced before a targeted repair. B2/C accepted the repaired runtime `64306e9`; D accepted its candidate with public-production checks still required. No estimator, gate or sensor contract changed. Root retains the distinction between alignment-conditioning hypotheses and measured accuracy, as well as the unresolved shared-heap growth.

Vercel `promote` rebuilt the candidate for production instead of reusing its bytes. Authenticated deployment metadata identifies action `promote`, original deployment `dpl_5NZXNY9uzmumMFvN8jkRmCVor9Rr`, and source/git commit `64306e97bbe632a7e06006842e1f49b8d53b7520`. The production deployment is `dpl_DsqUqYYtJekVNyey8ZSvneZm9FDH`, immutable URL `https://offline-event-horizon-hulpbadfy-sf-i455.vercel.app`. Six of eight entry-script hashes match; the Next/React bootstrap and Turbopack loader differ. Therefore this is recorded as the same reviewed source with separately tested production artifacts, not byte-identical preview promotion. The final public checks and D clarification govern release acceptance.

...
Candidate routes are local constant-curvature arcs, not world-space splines between attractions. Their selected curvature is converted to the craft's physical yaw rate at the chosen speed. The planner does not perform global search, guaranteed collision avoidance, learned aesthetic scoring or semantic recognition. Unknown near-field cells are rejected. A lack of a reliable opening leads to a small stationary scan and a long rest, rather than a blind journey. One scan window turns in place for up to 3.5 seconds at 0.8 of the shared steering constant, about 79 degrees, and the heading is retained between windows so the next scan opens a new sector. A scan that still finds nothing settles into a 30-60 second rest; a route followed for more than 18 seconds, extended by observed greenery, ends in a 32-58 second one. Commanded motion whose estimated position stays within 0.6 units for 4 seconds is read as a hull held against something and rests the same way. Only the estimated translation is consulted there: no truth pose, contact query or world coordinate enters the director. The craft can consequently remain in one area for a long time. Contact recovery is a separate last line of defense.
...
The replay is also an accuracy gate. It exits non-zero when a segment loses more than 0.1 of its samples, when endpoint drift exceeds 0.2 of the distance travelled, or when the RMS relative translation error exceeds 0.5; `REPLAY_MAX_LOST`, `REPLAY_MAX_DRIFT` and `REPLAY_MAX_RPE` override the three, and `scripts/pilgrim-lab.mjs` reads the same three variables, so one override moves the offline gate and the live lab gate together. A segment that travelled less than one unit reports no drift fraction and is gated by lost fraction alone. Neither retained real clip has ever produced an aligned ATE — both report their alignment as stationary or near-collinear — so the drift fraction and the one-second RPE are the meaningful numbers there. Reports are written to `artifacts/replay/` unless `--out` names a path.

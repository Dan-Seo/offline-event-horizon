...
Replay reports distinguish recorded-run agreement, evaluated accuracy gates, and metric availability. `inputSha256` hashes the archive exactly as read (compressed bytes for `.gz`), separately from any decoded TAR hash. Execution revision/dirty state describes the harness, not the recorded runtime. A status/inlier/delta mismatch writes the first failing capture and expected/actual values, exits nonzero, and leaves `evaluation=null` with accuracy gates explicitly not run. Counts distinguish completed status/inlier frames from relative-pose pairs. A successful short-fixture replay still exposes unavailable ATE/RPE and zero evaluated RPE segments; it is not evidence that every accuracy metric passed. The unchanged `rotation` value means aligned orientation RMS after positional SE(3) alignment, not independent heading accuracy.
